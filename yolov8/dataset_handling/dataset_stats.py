...
def overall_healthcheck(path_to_dataset: str, split: str = 'all', plot: bool = False):
    ...
    occurrences_dict = {label: occurrences_counter[index] if index in occurrences_counter
    else 0 for index, label in enumerate(class_names)}
    if plot:
        plot_on_barchart(occurrences_dict, f"Healthcheck: {split}",
