...
def save_one(frame: np.ndarray, coordinates: list[dict], save_location: str, file_name: str,
    ...
    index = 0
    while True:
        img_path = os.path.join(save_location, "images", file_name + f"_{index}" + image_extension)
        txt_path = os.path.join(save_location, "labels", file_name + f"_{index}" + ".txt")
